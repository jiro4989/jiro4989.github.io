...
| 色彩検定 2級 | 2021/12/8 ||
| AWSソリューションアーキテクトアソシエイト | 2022/8/6 | 2028/5/11 |
...
2年間のバックエンド開発経験もあり、バックエンドアプリの構成は大体把握している。
そのおかげでアプリとインフラ両方に改修を入れるSREとして働いている。

やったことを列挙すると以下。

1. 複数プロダクトで汎用的に利用できる terraform モジュールを管理するリポジトリ運用の設計・実装、及び自動テストの実装
1. アプリケーション自身がメンテナンス期間を判定して自動でメンテナンスモードに切り替わるライブラリの設計・実装・導入
1. tfstate 分割運用設計と実装
1. Terragrunt の検証・新規導入
1. 新規プロダクトの全体インフラ設計・実装
