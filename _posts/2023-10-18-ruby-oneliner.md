...
使える道具は多い方が芸の幅が広くなるので、覚えとくと役に立つ。
...
適切な変数名を設定し、可読性を意識して実装する必要がある。

ワンライナーはリポジトリ管理しないプログラムだから、使い捨てよう。

## ワンライナー用のオプション

まえがきが長くなったけれど、ここからようやくRubyワンライナーの説明をする。
...
ただし、こういった単純な正規表現での絞り込みのみであれば `grep` で済ませたほうが良い。
...
この時、Rubyのオプションは `-lane` ではなく `-e` だけ付与する。

CSVクラスは引数として標準入力を渡したあと、イテレータとして扱える。
標準入力をすべて渡すのであれば、`-e` だけで十分である。

### JSONの特定の列を取得

RubyではJSONクラスを使うことで、JSONを簡単に扱える。

最近JSON形式のログを扱うことが多い。
監視系がJSONパーサーを標準で備えているケースが多くなり、
それに合わせてログもJSON形式にすることも多くなったからだ。

そういった監視系に合わせたJSON形式のログの場合、1行に1つのJSONを出力する。
つまり、ワンライナーでJSONを扱う場合も、1行ずつ処理するのでほぼ問題ない。

例えば、1行1JSONのログから数値配列 `value` を取得して合算する例を示す。
...
前述の例は「1行に1つJSONが存在する」ものだったが「入力全体がJSON」の場合は、次のように実装する。
`JSON.parse` の引数は文字列型を受け付けるので、`STDIN` (IO) 型ではなく、`STDIN.read` を渡す必要がある。
...
Rubyワンライナーは非常に便利で、awkでできることはだいたいできる。
でも、何でもできるからといってすべてRubyでやるべきではない。
（これはawkやperlにも言えることだが）

1つのことをうまくやる専用のコマンドがほかに多数存在するので、専用コマンドでできることは任せたほうが良い。
正規表現で文字列を絞り込むなら `grep` がある。
文字列の置換なら `sed` がある。
ソートするなら `sort` がある。
これらのコマンドは歴史が長く、チューニングされ尽くしている。
パフォーマンスの面で見ても、専用コマンドを使ったほうが良い。

Rubyを使ってワンライナーで処理をする時は「Rubyでないと難しい」処理にのみ、Rubyを使うようにしよう。