...
Godot でゲームを作る前の、Godot に慣れる目的で作った GUI ツール。
ソートアルゴリズムの動きを可視化するもの。

Youtube でたまにソートアルゴリズムの動画を見ることがあって、
「この程度のツールならサクッと作れそう」と思って作ったもの。
だいたい作りたかった機能を実装できたので、今後アップデートすることはおそらくない。

作り始めてから初版ができるまでに 2 時間。
一通り満足するバージョンまで改修するのに 11 時間かかった。

プライベートで Cursor を本格的に使って実装した最初のプログラムでもある。
